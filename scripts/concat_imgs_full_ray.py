...
subdirectories = [d for d in (DATA_DIR/'raw').iterdir() if d.is_dir() and d.stem >= "study_room"]
ray.init()
my_id = [concat_img.remote(subdir) for subdir in subdirectories]
ray.get(my_id)